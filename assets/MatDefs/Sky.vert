...
uniform float m_ScaleDepth;  //(outerRadius - innerRadius) / 2f;
...
void main() {
    ...
    if(m_LightDir.y >= -0.3){
        ...
        float fDepth = exp(m_ScaleOverScaleDepth * (m_InnerRadius - m_CameraPos.y));//负数
        //相机沿观察方向到大气层顶部的光学距离（AB）
        ...
        for (int i = 0; i < m_NumberOfSamples; i++)
        {
            ...
            float fDepth = exp(m_ScaleOverScaleDepth * (m_InnerRadius-fHeight));//负数，exp(-h/H0)
